...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            query_key = f"{ROOT}/{ATTENTION_Q}/kernel"
            key_key = f"{ROOT}/{ATTENTION_K}/kernel"
            value_key = f"{ROOT}/{ATTENTION_V}/kernel"
            out_key = f"{ROOT}/{ATTENTION_OUT}/kernel"
    ...
    def __init__(self, config):
        ...
        blocks = [
            DecoderBlock(
                ...
                out_ch,
                sk_ch,
                attention_type=config.attention_type  # 传递配置
...
class MultiScaleFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        # 在不同尺度下进行融合，首先对 x2 进行上采样与 x1 对齐
        x2_rescaled = F.interpolate(x2, size=(x1.size(2), x1.size(3)), mode='bilinear', align_corners=True)
        x_fused = torch.cat([x1, x2_rescaled], dim=1)  # 拼接
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.refine_cbam = CBAMLayer(config.decoder_channels[-1])
        ...
        # 中间监督分割头（对 decoder1 输出）
        self.aux_segmentation_head = SegmentationHead(
            in_channels=config.decoder_channels[-1],
            out_channels=config['n_classes'],
            kernel_size=3,
        )
    ...
    def forward(self, x):
        ...
        x1 = self.decoder1(x, features)  # decoder1 输出
        aux_logits = self.aux_segmentation_head(x1)  # 中间 supervision 输出
        x = self.refine_cbam(x1)
        x = self.decoder2(x)
        final_logits = self.segmentation_head(x)
